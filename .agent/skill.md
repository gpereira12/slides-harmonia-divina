...
| Liturgical/Church | 1 stanza maximum per slide (~42px font size) |

**If content exceeds these limits → Split into multiple slides**

### ⛪ Special Case: Liturgical / Church Presentions Logic
When formatting slides for Church Songs (e.g. from Notion/APIs), follow these exact rules:

**1. Layout & Typography**:
- **Font Size**: The base font size MUST be explicitly large to maintain legibility from afar. Set the root variable: `--body-size: clamp(2rem, 4vw, 3.5rem)` (~42px base on desktop).
- **Width Scenarios**: The `.slide-content` or main container MUST have two specific max-width scenarios:
  - Default Desktop: `max-width: 1024px; margin: 0 auto;`
  - Mobile/Narrow: `@media (max-width: 768px) { .slide-content { max-width: 600px; } }`

**2. Standard Lyric Processing**:
- **Empty Lyrics**: If a song has no translated lyrics, skip the song entirely.
- **Numbered Stanzas**: If standard text contains numerical lists (e.g. `1. `, `2. `), isolate each numbered point as its own separate slide.

**3. Chorus & Interleaving (The "Entrada/Comunhão/Final" Pattern)**:
- Whenever lyrics contain **bold** annotations, the FIRST bold block is treated as the *Global Chorus (Refrão)*.
- **Default Behavior**: Interleave this Chorus before *every* Stanza slide (Chorus -> Stanza 1 -> Chorus -> Stanza 2).

**4. MANDATORY EXCEPTIONS**:
- **Ato Penitencial (Fórmula 3)**: Render blocks *sequentially* exactly as they appear in the rich text (Estrofe -> Refrão Kyrie -> Estrofe -> Refrão Christe). **Do NOT** interleave a global chorus.
- **Ato Penitencial (Fórmula 2)**: The soloist (normal text) and the crowd (bold text) MUST be rendered on the **same slide** (e.g., "Kyrie Eleison `<br>` **Kyrie Eleison**" on one slide). 
- **Aclamação ao Evangelho**: ONLY render the **bold** text (the acclamation). Completely ignore/discard all normal text (the verses).
- **Santo**: Render blocks *sequentially*. Output the first bold chorus (Santo, Santo...) once, followed by the normal stanzas once. **Do NOT** interleave or repeat the chorus.
